...
<p align="center">
  <img src="docs/images/agent-pending-zh.png" width="420" alt="Agent Pending 中文运行界面">
</p>
...
- 每项显示标题、一个明确的下一步、重要程度、工作区名称和记录时间；可一键复制完整工作区路径。
- 拖动卡片调整处理先后；“高 / 中 / 低”只表示重要程度，不会自动改变顺序。
- 可在界面中编辑标题、待处理内容、重要程度和工作目录，或标记完成并移入归档。
- 使用原生 macOS 材质、系统字体和动态系统色，自动适配浅色与深色外观。
- CLI 支持新增、查看、排序、调整重要程度、归档和恢复；相同记录默认不会重复添加。
...
记录包含标题、下一步、工作区路径、时间、处理顺序和重要程度。顺序决定先处理哪项，重要程度只作判断提示；工作区路径只是定位信息，App 和 Skill 不会因此读取项目内容。

## 不会做什么

Agent Pending v0.2 有意不提供：
...
v0.2 会在本机编译并进行 ad-hoc 签名；仓库暂不提供经过 Apple 公证的二进制安装包。
